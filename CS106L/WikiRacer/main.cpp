...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    ...
    if (curr_set.size() == 0 || target_set.size() == 0) return 0;
    int ans;

    auto begin = target_set.begin();
    auto end = target_set.end();
    auto i = curr_set.begin();
    while(i != curr_set.end()){
        if (find(begin, end, *i) != end) ans++;
        i++;
    }
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    auto cmp_fn = [&w, &target_set](const vector<string>& left, const vector<string>& right) {
        ...
        unordered_set<string> set1 = w.getLinkSet(*(left.end()-1));
        unordered_set<string> set2 = w.getLinkSet(*(right.end()-1));
        return (numCommonLinks(set1, target_set) < numCommonLinks(set2, target_set)); // replace this line! make sure to use numCommonLinks.
    };
    // END STUDENT CODE HERE
    ///////////////////////////////////////////////////////////////////////////////////////////////////
