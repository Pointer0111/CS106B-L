...
Nucleotide* toStrand(const string& str) {
    ...
    for (int i = 0; i < n; i++){
        Nucleotide* y = new Nucleotide();
        x->value = str[i];
        x->next = y;
        y->prev = x;
        if (i == n - 1) x->next = nullptr;
        x = y;
    }
    delete x;
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    if (!target) return dna;
    if (!dna) return nullptr;
    Nucleotide* p = dna;
    Nucleotide* q = target;
    if (!q->next){
        while (p){
            if (p->value == q->value) return p;
            p = p->next;
        }
        return nullptr;
    }
        ...
        while(p->next && q->next){
            if ((p->value == q->value)&&(p->next->value == q->next->value)){
                p = p->next;
                q = q->next;
            }
            else {
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* p = findFirst(dna, target);
    if (!p) return false;
    Nucleotide* q = target;
    Nucleotide* tmp = p;
    Nucleotide* prev = p->prev;
