...
bool RobinHoodHashTable::insertRec(const string& elem, int pos, int dis){
    // 情况一：找到一个空桶
    if (elems[pos].distance == EMPTY_SLOT){
        elems[pos].value = elem;
        elems[pos].distance = dis;
        logicalSize++;
        return true;
    }
    // 情况二：对桶内元素进行替换
    else if (dis > elems[pos].distance){
        string a = elems[pos].value;
        int b = elems[pos].distance;
        elems[pos].value = elem;
        elems[pos].distance = dis;

        while (1){
            if (this->insertRec(a, pos, b)) break;
            else {
...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (this->contains(elem) || logicalSize == allocatedSize) return false;
    int n = hashFn(elem);
    int dis = 0;
    while (1){
        if (this->insertRec(elem, n, dis)) return true;
        else {
            n = (n + 1)% allocatedSize;
            dis++;
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int i = 0;
    int dis = 0;
    while (i < allocatedSize){
        n = n % allocatedSize;
        if (elems[n].distance != EMPTY_SLOT && elems[n].value == elem) return true;
        else if (elems[n].distance == EMPTY_SLOT || dis > elems[n].distance) return false;
        i++;
        n++;
        dis++;
    }
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (i < allocatedSize){
        n = n % allocatedSize;
        if (elems[n].distance != EMPTY_SLOT && elems[n].value == elem) {
            elems[n].distance = EMPTY_SLOT;
            logicalSize--;
            while(1){
                m = (n+1)%allocatedSize;
                if (elems[m].distance == EMPTY_SLOT||elems[m].distance == 0) break;
                else{
                    elems[n].value = elems[m].value;
                    elems[n].distance = elems[m].distance - 1;
                    elems[m].distance = EMPTY_SLOT;
                    n = m;
                }
            }
            return true;
