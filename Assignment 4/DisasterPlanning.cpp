...
bool DPRec(const Map<string, Set<string>>& roadNetwork,
    ...
    for (string key: roadNetwork) {
        m = 0;
        if (supplyLocations.contains(key)){
            sum += 1;
            continue;
        }
        for (string loc: roadNetwork[key]){
            if (supplyLocations.contains(loc)){
                m = 1;
                break;
            }
        }
    ...
    if (sum == roadNetwork.size()) return true;


    string x = incomplete.first();
    Map<string, Set<string>> remaining;
    remaining += roadNetwork;
    remaining.remove(x);
    if (supplyLocations.contains(x)){
        if (DPRec(remaining, numCities, supplyLocations)) return true;
    }
    else{
        supplyLocations.add(x);
        if (DPRec(remaining, numCities - 1, supplyLocations)) return true;
        else supplyLocations.remove(x);
    }


    for (string y:roadNetwork[x]){
        Map<string, Set<string>> remaining1;
        remaining1 += remaining;
        remaining1.remove(y);
        if (supplyLocations.contains(y)) {
            if (DPRec(remaining1, numCities, supplyLocations)) return true;
        }
        else {
            supplyLocations.add(y);
            if (DPRec(remaining1, numCities - 1, supplyLocations)) return true;
            else supplyLocations.remove(y);
        }
    }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if (roadNetwork.size() < numCities){
        for (string key: roadNetwork) supplyLocations.add(key);
        return true;
    }

    return DPRec(roadNetwork, numCities, supplyLocations);
