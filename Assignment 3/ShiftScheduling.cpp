...
bool isInvalid(Shift shift, const Set<Shift> &shifts, const int maxHours){
    int sum = lengthOf(shift);
    if (sum > maxHours) return true;
...
Set<Shift> hvalue(const Set<Shift> &shifts, const Set<Shift> &soFar, const int maxHours){
    if (shifts.isEmpty()) return soFar;
    else {
        Shift shift = shifts.first();
        Set<Shift> remain = shifts - shift;
        Set<Shift> s2 = hvalue(remain, soFar, maxHours);
        if (isInvalid(shift, soFar, maxHours)) return s2;
        else{
            int sum1 = 0, sum2 = 0;
            Set<Shift> s1 = hvalue(remain, soFar + shift, maxHours);
            for (Shift a: s1){
                sum1 += valueOf(a);
            }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) error("maxHours should be positive!");
    Set<Shift> soFar;
    if (maxHours == 0) return soFar;
    return hvalue(shifts, soFar, maxHours);
}
