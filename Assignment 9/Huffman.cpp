...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    int n = str.size();
    for (int i = 0; i < n; i++){
        if (!mymap.containsKey(str[i])) mymap.put(str[i], 1);
        else mymap[str[i]]++;
    }

    if (mymap.size() < 2) error("You should input at least two different characters!");

    for (char c: mymap){
        EncodingTreeNode* x = new EncodingTreeNode();
        x->ch = c;
        x->one = nullptr;
        x->zero = nullptr;
        pq.enqueue(x, mymap[c]);
    }

    while (pq.size() > 1){
        double a = pq.peekPriority();
        EncodingTreeNode* y = pq.dequeue();
        double b = pq.peekPriority();
        EncodingTreeNode* z = pq.dequeue();
        EncodingTreeNode* x = new EncodingTreeNode();
        x->zero = y;
        x->one = z;
        pq.enqueue(x, a+b);
 ...
 */
void dictionary(EncodingTreeNode* tree, Vector<Bit> &vec, Map<char, Vector<Bit>> & mymap){
    // 递归实现
    if (!tree->one && !tree->zero){
        mymap.put(tree->ch, vec);
        return;
    }
    else {
        Vector<Bit> vec1;
        vec1 += vec;
        vec1.add(0);
        dictionary(tree->zero, vec1, mymap);

        Vector<Bit> vec2;
        vec2 += vec;
        vec2.add(1);
        dictionary(tree->one, vec2, mymap);
    }
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Queue<Bit> bits;
    // 我们需要一个霍夫曼编码与字符对应的表
    Map<char, Vector<Bit>> mymap;
    Vector<Bit> ver;
    dictionary(tree, ver, mymap);

    int n = str.size();
    for (int i = 0; i < n; i++){
        Vector<Bit> vec = mymap[str[i]];
        int m = vec.size();
        for (int j = 0; j < m; j++) bits.enqueue(vec[j]);
    }
...
string decompress(HuffmanResult& file) {
    ...
    string s = decodeText(file.messageBits, tree);
