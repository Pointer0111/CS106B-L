...
bool hasPMRecur(const Map<string, Set<string>>& possibleLinks,
                Set<Pair>& matching, Set<string> &allocated){
    if (possibleLinks.size() == 0) return true;
    string x = possibleLinks.firstKey();
    for (string y: possibleLinks[x]){
        if (!allocated.contains(y)){
            Map<string, Set<string>> remaining;
            remaining += possibleLinks;
            remaining.remove(x);
            remaining.remove(y);
            allocated.add(x);
            allocated.add(y);
            if (hasPMRecur(remaining, matching, allocated)){
                Pair pr(x, y);
                matching += pr;
                break;
            }
    ...
    if (!allocated.contains(x)) return false;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    if (possibleLinks.size()%2 != 0) return false;
    for (string key: possibleLinks) if (possibleLinks[key].size() == 0) return false;
    Set<string> allocated;
    return hasPMRecur(possibleLinks, matching, allocated);

}

int MWMRecur(const Map<string, Map<string, int>>& possibleLinks, Set<string> &visited, Set<Pair> &ans){
    //跳出递归条件
    int m = 0;
    for (string a: possibleLinks){
        for (string b: possibleLinks[a]){
            if (!visited.contains(b) && possibleLinks[a][b] > 0){
                m = 1;
                break;
            }
        }
    ...
    visited.add(x);
    remaining.remove(x);
    scores.enqueue("", (double) -MWMRecur(remaining, visited, ans));

    for (string y: possibleLinks[x]){
        if (!visited.contains(y) && possibleLinks[x][y] > 0){
            visited.add(y);
            Map<string, Map<string, int>> remaining1;
            remaining1 += remaining;
            remaining1.remove(y);
            Set<Pair> ans1;
            int score = MWMRecur(remaining1, visited, ans1) + possibleLinks[x][y];
            scores.enqueue(y, (double) -score);
            visited.remove(y);
            if (scores.peek() == y){
                Pair pr(x, y);
                ans = ans1 + pr;
    ...
    return (int) -scores.peekPriority();
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Set<Pair> ans;
    Set<string> visited;
    int score = MWMRecur(possibleLinks, visited, ans);
    return ans;
}
