...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength < 1)  error("The length of grams must be positive!");
    Map<string, double> mymap;
    string s;
    if (kGramLength > str.size()) return mymap;
    int n = str.size();
    for (int i = 0; i < n-kGramLength+1; i++){
        s = charToString(str[i]);
        for (int j = 1; j < kGramLength; j++) s.push_back(str[i + j]);
        if (mymap.containsKey(s)) mymap[s] += 1;
        else mymap.put(s, 1);
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key : input) {
        quadsum += pow(input[key], 2);
    }
    if (quadsum == 0) error("Invalid Input!");
    double nom = sqrt(quadsum);
    mymap += input;

    for (string key : mymap) {
        mymap[key] = mymap[key]/nom;
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) error("Invalid Argument!");
    if (numToKeep >= source.size()) return source;
    Map<string, double> mymap;
    if (numToKeep == 0) return mymap;
    PriorityQueue<string> pq;
    for (string key: source){
        pq.enqueue(key, source[key]);
    }
    int n = source.size() - numToKeep;
    string s;
    mymap += source;
    for (int i = 0; i < n;i++){
        s = pq.dequeue();
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key: lhs){
        if (rhs.containsKey(key)) sum += lhs[key]*rhs[key];
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    PriorityQueue<string> pq;
    if (corpora.size() == 0) error("Corpora is Empty!");
