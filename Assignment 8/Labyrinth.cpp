...
bool isPathToFreedom(MazeCell* start, const string& moves) {
    MazeCell* loc = start;
    Set<Item> st;
    if (loc->whatsHere != Item::NOTHING) st.add(loc->whatsHere);
    int n = moves.size();
    for (int i = 0; i < n; i++){
        char c = moves[i];
        switch(c){
        case 'N':
            if (!loc->north) return false;
            else loc = loc->north;
            break;
        case 'W':
            ...
            break;
        case 'S':
            if (!loc->south) return false;
            else loc = loc->south;
            break;
        case 'E':
            if (!loc->east) return false;
            else loc = loc->east;
            break;
    ...
    if (st.contains(Item::SPELLBOOK) && st.contains(Item::POTION) && st.contains(Item::WAND)) return true;
    else return false;
}
