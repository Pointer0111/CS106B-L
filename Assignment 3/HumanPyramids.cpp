...
double weight(int row, int col, HashMap<GridLocation, double> &mymap){

    GridLocation loc(row, col);
    if (mymap.containsKey(loc)) return mymap[loc];
        ...
        double sum = 0;
        sum += weight(row-1, col, mymap) + weight(row-1, col-1, mymap) + 320;
        mymap.put(loc, sum/2);
        return sum/2;
    }
}
...
double weightOnBackOf(int row, int col, int pyramidHeight) {
    if (row < 0 || col < 0 || col > row ||row > pyramidHeight) error("Invaild numRow or numCol");
    HashMap<GridLocation, double> mymap;
    return weight(row, col, mymap);
}
