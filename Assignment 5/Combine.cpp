...
Vector<DataPoint> merge(Vector<DataPoint> vec1, Vector<DataPoint> vec2){
    if (vec1.size() == 0) return vec2;
    if (vec2.size() == 0) return vec1;
    Vector<DataPoint> vec;
    int i = 0, j = 0;
    while (i < vec1.size() && j < vec2.size()){
        if (vec1[i].weight < vec2[j].weight) vec.add(vec1[i++]);
        else vec.add(vec2[j++]);
    }

    while (i < vec1.size()) vec.add(vec1[i++]);
    while (j < vec2.size()) vec.add(vec2[j++]);
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> vec;
    if (sequences.size() == 0) return vec;
    if (sequences.size() == 1) return sequences[0];
    else if (sequences.size() == 2) return merge(sequences[0], sequences[1]);
    else{
        int n = sequences.size();
        Vector<Vector<DataPoint>> seq1 = sequences.subList(0, n/2);
        Vector<Vector<DataPoint>> seq2 = sequences.subList(n/2, n - n/2);
        Vector<DataPoint> vec1 = combine(seq1);
        Vector<DataPoint> vec2 = combine(seq2);
