...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    while (i < allocatedSize){
        n = n % allocatedSize;
        if (elems[n].type == SlotType::EMPTY || elems[n].type == SlotType::TOMBSTONE){
            elems[n].value = elem;
            elems[n].type = SlotType::FILLED;
            logicalSize++;
            return true;
        }
        i++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    while (i < allocatedSize){
        n = n % allocatedSize;
        if (elems[n].type == SlotType::FILLED && elems[n].value == elem) return true;
        else if (elems[n].type == SlotType::EMPTY) return false;
        i++;
        n++;
    }
    return false;
...
bool LinearProbingHashTable::remove(const string& elem){
    ...
    while (i < allocatedSize){
        n = n % allocatedSize;
        if (elems[n].type == SlotType::FILLED && elems[n].value == elem) {
            elems[n].type = SlotType::TOMBSTONE;
            logicalSize--;
            return true;
        }
        i++;
