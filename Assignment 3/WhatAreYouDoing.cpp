...
void allComb(Set<string> &st, const Vector<string> &vec, int i, string s){
    if (i == vec.size()){
        st.add(s);
        s = {};
        return;
    ...
    if (isalpha(vec[i][0])){
        string s1 = toLowerCase(vec[i]);
        string s2 = toUpperCase(vec[i]);
        allComb(st, vec, i + 1, s + s1);
        allComb(st, vec, i + 1, s + s2);
    }
    else allComb(st, vec, i + 1, s + vec[i]);
