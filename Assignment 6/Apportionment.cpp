...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    if (populations.size() > numSeats) error("the number of states is greater than the number of seats");
    HeapPQueue myheap;
    Map<string, int> ans;
    for (string state: populations) {
        ...
        DataPoint dp;
        dp.name = state;
        // 注意要求是大根堆，因此入列时应把参数取负
        dp.weight = - populations[state] / sqrt(2);
        myheap.enqueue(dp);
    }
    numSeats -= populations.size();

    while (numSeats > 0){
        DataPoint x = myheap.dequeue();
        ans[x.name]++;
        int num = ans[x.name];
        num = num*(num+1);
        numSeats--;
        x.weight = - populations[x.name] / sqrt(num);
        myheap.enqueue(x);
    }
