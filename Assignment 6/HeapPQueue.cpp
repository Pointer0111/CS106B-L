...
void swap1(DataPoint &a, DataPoint &b){
    DataPoint tmp = b;
    b = a;
    a = tmp;
}
...
DataPoint HeapPQueue::peek() const {
    /* TODO: Delete the next line and implement this. */
    if (logicalSize == 0) error("The heap is empty!");
    return elems[1];
}
...
DataPoint HeapPQueue::dequeue() {
    ...
    if (logicalSize == 0) error("The heap is empty!");
    DataPoint x = elems[1];
    swap1(elems[1], elems[logicalSize]);
    logicalSize -= 1;

    int i = 1, m;
    while (i <= logicalSize/2){
        if (2*i + 1 <= logicalSize) m = (elems[2 * i].weight < elems[2*i + 1].weight)?2*i :2*i + 1;
        else m = 2*i;
        if (elems[i].weight > elems[m].weight){
            swap1(elems[i], elems[m]);
            i = m;
        }
        else break;
    }
